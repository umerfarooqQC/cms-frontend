...
export const CategoryTable = ({ data = [],getData }) => {
  ...
  const ActionMenu = ({id}) => {
    return (
      <Menu anchorEl={anchorEl} open={open} onClose={handleClose}>
        ...
        <MenuItem sx={{display:"flex",justifyContent:"space-between",alignItems:"center"}} onClick={(e)=>{handleDeleteCategory(id)}}><span>Delete</span><IconButton color="error" size="small"><Delete /></IconButton></MenuItem>
        <MenuItem sx={{display:"flex",justifyContent:"space-between",alignItems:"center"}} onClick={()=>{getSelectedTemplateData(id);setTempModalOpen(true); setAnchorEl(null)}}><span>View Templates</span><IconButton color="info" size="small"><FilterList /></IconButton></MenuItem>
      </Menu>
    );
  };
  ...
  return (
    <>
        ...
        {data?.length && (
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableRow sx={{ width: "100%" ,background:"#1A73E8"}}>
              ...
              <TableHeaderCell isFirstColumn={true}>COUNT</TableHeaderCell>
            ...
            <TableBody>
              {data.map((row) => (
                <TableRow key={row?.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                  ...
                  <TableCell align="left">{row?.templates_count}</TableCell>
      ...
      <ActionMenu id={id} />
      <TemplateListModal open={tempModalOpen} data={templateData} setData={setTemplateData} confirmOpen={confirmOpen} setConfirmOpen={setConfirmOpen} onDelete={handleDeleteTemplate} catId={id} setOpen={setTempModalOpen}/>
      <ToastContainer />
    </>
  );
};
